package com.radlly.util.format;
...
import com.radlly.exception.ExceptionStatus;
...
public class DateUtils {
	...
	public static Date parseDate(String strDate, String pattern) {
		try {
			...
			throw new BusinessException(ExceptionStatus.DATE_ERROR);
		}
	}
        ...
        if(dates != null && dates.length > 0){
            for(String dateStr : dates){
                if(!StringHelper.isEmpty(dateStr)) {
                    ...
                    if(strs.length == 1){
                        checkDateFormat(strs[0], DateUtils.YYYYMMDD);
                    }else if(strs.length == 2){
                        checkDateFormat(dateStr, DateUtils.YYYYMMDD2);
                    }
                }
            }
        }
     ...
     * @param f
     */
    private static void checkDateFormat(String dateString, String f){
        try {
